...
export class OtpCodeMailQueueJob extends QueueJobBase {
  ...
  public handle(job: Job) {
    ...
    mailService.subject(`${appConfig.appName} - Login`).to(this._data.email).text(this._data.content).sendWithoutErrorHandling();
